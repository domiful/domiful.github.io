...
const renderComponent = (props = {}) => render(<RepoAboutItem {...props} />);

describe.only('<RepoAboutItem />', () => {
  let item;
  ...
  it('should render a AboutItem', () => {
    const renderedComponent = shallow(<RepoAboutItem item={item} />);
    expect(renderedComponent.find(AboutItem).length).toBe(1);
  });
